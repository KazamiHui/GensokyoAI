...
class CommandExecutor:
    ...
    def _log_execution(
        ...
        if result.status == CommandStatus.SUCCESS:
            if result.message:
                logger.info(f"[{context.source.upper()}] Command '{parsed.name}' succeeded: {result.message}")
        elif result.status == CommandStatus.FAILURE:
            logger.warning(f"[{context.source.upper()}] Command '{parsed.name}' failed: {result.message}")
        else:
